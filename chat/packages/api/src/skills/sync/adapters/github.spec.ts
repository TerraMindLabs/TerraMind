...
describe('createGitHubRepoAdapter', () => {
  describe('resolveCommit', () => {
    it('resolves the configured ref to its commit and root tree', async () => {
      ...
      expect(fetchFn).toHaveBeenCalledWith(
        'https://api.github.com/repos/TerraMind/skills/commits/main',
        {
          headers: expect.objectContaining({
            Accept: 'application/vnd.github+json',
            Authorization: 'Bearer github_pat_secret',
            'X-GitHub-Api-Version': '2022-11-28',
            'User-Agent': 'TerraMind-Skill-Sync',
          }),
        },
      );
    });
    ...
    it('encodes each ref segment without escaping its separators', async () => {
      ...
      expect(fetchFn).toHaveBeenCalledWith(
        'https://api.github.com/repos/TerraMind/skills/commits/release/v1%20rc',
        expect.anything(),
      );
    });
  });
  ...
  describe('fetchTreeEntries', () => {
    it('lists the whole repository from the root tree when no path is configured', async () => {
      ...
      expect(fetchFn).toHaveBeenCalledWith(
        'https://api.github.com/repos/TerraMind/skills/git/trees/tree-sha?recursive=1',
        expect.anything(),
      );
    });
    ...
    it('walks one non-recursive listing per path segment before listing recursively', async () => {
      ...
      expect(urls).toEqual([
        'https://api.github.com/repos/TerraMind/skills/git/trees/tree-sha',
        'https://api.github.com/repos/TerraMind/skills/git/trees/skills-tree-sha',
        'https://api.github.com/repos/TerraMind/skills/git/trees/shared-tree-sha?recursive=1',
      ]);
    });
  ...
  describe('fetchFileContent', () => {
    it('decodes base64 blob content, ignoring the wrapping whitespace GitHub inserts', async () => {
      ...
      expect(fetchFn).toHaveBeenCalledWith(
        'https://api.github.com/repos/TerraMind/skills/git/blobs/skill-md-sha',
        expect.anything(),
      );
    });
