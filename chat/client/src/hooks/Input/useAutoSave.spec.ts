...
const markTabLive = (tabId: string): void =>
  localStorage.setItem(`TerraMind-live-tab:${tabId}`, JSON.stringify({ seenAt: Date.now() }));

const markTabGone = (tabId: string): void => localStorage.removeItem(`TerraMind-live-tab:${tabId}`);
