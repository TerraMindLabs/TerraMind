...
//#region src/admin/capabilities.ts
/**
...
const impliedByMap = {};
for (const [manage, reads] of Object.entries(CapabilityImplications)) for (const read of reads) {
	if (!impliedByMap[read]) impliedByMap[read] = [];
...
function hasImpliedCapability(held, required) {
	...
	if (impliers) {
		for (const cap of impliers) if (held.includes(cap)) return true;
	}
	return false;
}
/**
...
function expandImplications(directCaps) {
	...
	for (const cap of directCaps) {
		...
		if (implied) for (const imp of implied) expanded.add(imp);
	}
...
const ResourceCapabilityMap = {
	[librechat_data_provider.ResourceType.AGENT]: SystemCapabilities.MANAGE_AGENTS,
	[librechat_data_provider.ResourceType.CODE_ENVIRONMENT]: SystemCapabilities.MANAGE_CODE_ENVIRONMENTS,
	[librechat_data_provider.ResourceType.PROMPTGROUP]: SystemCapabilities.MANAGE_PROMPTS,
	[librechat_data_provider.ResourceType.MCPSERVER]: SystemCapabilities.MANAGE_MCP_SERVERS,
	[librechat_data_provider.ResourceType.REMOTE_AGENT]: SystemCapabilities.MANAGE_AGENTS,
	[librechat_data_provider.ResourceType.SKILL]: SystemCapabilities.MANAGE_SKILLS,
	[librechat_data_provider.ResourceType.SHARED_LINK]: SystemCapabilities.MANAGE_SHARED_LINKS
};
/**
