...
describe('ChatTitleInTab', () => {
  beforeEach(() => {
    localStorage.clear();
    localStorage.setItem(LocalStorageKeys.APP_TITLE, 'TerraMind');
  ...
  it('keeps the app title when enabling titles for a new chat', () => {
    localStorage.setItem(LocalStorageKeys.APP_TITLE, 'Custom TerraMind');
    document.title = 'Custom TerraMind';
    renderToggle({
      route: '/c/new',
  ...
  it('preserves page-specific titles outside chat routes', () => {
    ...
    expect(document.title).toBe('Agent Marketplace | TerraMind');
  });
});
