...
describe('requireJwtAuth tenant context chaining', () => {
  ...
  it('uses OpenID JWT before TerraMind JWT when the OpenID cookie is present', async () => {
    isEnabled.mockReturnValue(true);
    mockRegisteredStrategies.add('openidJwt');
    const req = mockReq(undefined, {
...
describe('requireRumProxyAuth', () => {
  ...
  it('authenticates telemetry with the TerraMind JWT strategy without tenant or cookie refresh middleware', () => {
  ...
  it('falls back to TerraMind JWT when OpenID JWT telemetry auth fails', () => {
    isEnabled.mockReturnValue(true);
    mockRegisteredStrategies.add('openidJwt');
    const req = mockReq(undefined, {
      headers: { cookie: `token_provider=openid; openid_user_id=${signedOpenIdUserCookie()}` },
