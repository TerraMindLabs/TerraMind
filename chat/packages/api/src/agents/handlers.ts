...
export interface ToolExecuteOptions {
   ...
   * carry the TerraMind step stream (Open Responses), which simply skips the
   * instrumentation.
   */
    ...
    if (hasActiveFileFieldPolicy(req?.config?.filters, ['content', 'extracted_text'])) {
      ...
      try {
        ...
        if (error instanceof WorkspaceToolHttpError && error.upstreamStatus === 400) {
          error.message +=
            '. This attached environment must update its TerraMind Code worker before protected files can be edited.';
        }
        throw error;
      }
