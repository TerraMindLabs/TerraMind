...
(async () => {
  ...
  if (!hasSharedGenerationStore) {
    const confirmOffline = await askQuestion(
      'Shared Redis generation coordination is unavailable. Confirm ALL TerraMind app, worker, and other deletion CLI processes are stopped before continuing. (y/N)',
    );
    if (confirmOffline.toLowerCase() !== 'y') {
      console.yellow('Aborted. Stop every TerraMind process or enable Redis stream coordination.');
    ...
    if (fenceState === 'in_progress') {
      ...
      if (!allProcessesStopped) {
        const confirmRecovery = await askQuestion(
          'An account-deletion fence already exists. Confirm ALL TerraMind app, worker, and other deletion CLI processes are stopped to recover it only if stale. (y/N)',
        );
