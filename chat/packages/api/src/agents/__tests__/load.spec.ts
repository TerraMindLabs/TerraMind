...
const deps: LoadAgentDeps = {
  getAgent: (searchParameter) => getAgent(searchParameter) as Promise<TerraMindAgent | null>,
    ...
    const result = await loadAddedAgent(
      {
        ...
        primaryAgent: { id: EPHEMERAL_AGENT_ID as string, tools: ['web_search'] } as TerraMindAgent,
      },
      deps,
    );
