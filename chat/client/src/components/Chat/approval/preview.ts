...
export function buildApprovalPreview(
  ...
  } else if (request.source === 'TerraMind_code' && request.name === 'create_file' && parsed) {
    kind = 'create';
  ...
  } else if (request.source === 'TerraMind_code' && request.name === 'edit_file' && parsed) {
    kind = 'edit';
    target = stringField(parsed, 'path', 'file_path') || undefined;
