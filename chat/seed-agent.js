...
async function run() {
  try {
    ...
    for (const skillData of SKILLS) {
      const existing = await skillsCol.findOne({ name: skillData.name });
      let skillId;
      if (existing) {
        ...
        await skillsCol.updateOne(
          ...
          {
            $set: {
              displayTitle: skillData.displayTitle,
        ...
        const res = await skillsCol.insertOne(doc);
        skillId = res.insertedId;
      }

      skillIdMap[skillData.name] = skillId.toString();
