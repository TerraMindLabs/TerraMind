...
function createMocks(overrides?: {
  ...
  mockExtractTerraMindParams.mockReturnValue({
    resendFiles: false,
    maxContextTokens,
    modelOptions: { model },
  });
...
describe('initializeAgent — provider web_search precedence', () => {
  ...
  it('keeps Anthropic native web_search when TerraMind search is not selected', async () => {
    const { agent, req, res, loadTools, db } = createMocks({
  ...
  it('keeps TerraMind web_search definitions when native Anthropic search is not enabled', async () => {
    const { agent, req, res, loadTools, db } = createMocks({
      provider: Providers.ANTHROPIC,
  ...
  it('prefers TerraMind web_search when Anthropic native search is also enabled', async () => {
    const { agent, req, res, loadTools, db } = createMocks({
      provider: Providers.ANTHROPIC,
      providerTools: [nativeWebSearchTool],
      loadedToolDefinitions: [libreChatWebSearchDefinition],
  ...
  it('keeps Google native search when TerraMind web_search is not selected', async () => {
    const { agent, req, res, loadTools, db } = createMocks({
  ...
  it('prefers TerraMind web_search when Google native search is also enabled', async () => {
    const { agent, req, res, loadTools, db } = createMocks({
      provider: Providers.GOOGLE,
      providerTools: [nativeGoogleSearchTool],
      loadedToolDefinitions: [libreChatWebSearchDefinition],
  ...
  it('preserves the Google urlContext tool when TerraMind web_search is also enabled', async () => {
    /**
     * url_context is unrelated to web search, so the TerraMind web_search conflict
...
describe('initializeAgent — attachment scoping', () => {
  ...
  it('applies aggregate limits only to endpoint-compatible file survivors', async () => {
    ...
    const { agent, req, res, loadTools, db } = createMocks();
    mockExtractTerraMindParams.mockReturnValueOnce({
      resendFiles: true,
...
describe('initializeAgent — code-generated file thread filter (regression)', () => {
  ...
  beforeEach(() => {
    jest.clearAllMocks();
    mockExtractTerraMindParams.mockReset();
