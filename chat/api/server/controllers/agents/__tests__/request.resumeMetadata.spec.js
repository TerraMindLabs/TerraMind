...
jest.mock('@librechat/api', () => ({
  ...
  EVENT_ACTOR_DETACHED_COMPLETION_SOURCE: 'TerraMind-event-actor',
  EVENT_ACTOR_DETACHED_COMPLETION_TYPE: 'TerraMind.event_actor.detached_completion',
...
describe('ResumableAgentController resume metadata', () => {
  ...
  it('persists and exactly echoes protocol v2 on a newly created generation', async () => {
    ...
    expect(res.set).toHaveBeenCalledWith('x-TerraMind-generation-protocol', '2');
    expect(res.json).toHaveBeenCalledWith({
      streamId: 'conversation-123',
      conversationId: 'conversation-123',
      generationCreatedAt: 1000,
  ...
  it('caps a reacquired v2 lease to the immutable v1 live-job protocol', async () => {
    ...
    const req = {
      user: { id: 'user-123' },
      headers: { 'x-TerraMind-generation-protocol': '2' },
      body: {
    ...
    const req = {
      ...
      body: {
        ...
        agentEventDelivery: {
          ...
          event: {
            id: 'task-detached-1',
            type: 'TerraMind.event_actor.detached_completion',
            occurredAt: Date.now(),
            source: { id: 'TerraMind-event-actor', type: 'internal' },
          },
          internalCompletion: {
            version: 1,
            invocationId: 'original-delivery-1',
