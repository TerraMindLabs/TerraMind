 ...
 *
 * These handlers convert TerraMind's internal graph events into OpenAI-compatible
...
export class OpenAIRunStepHandler implements EventHandler {
  ...
  handle(_event: string, data: { stepDetails?: { type?: string } }): void {
    // Run step events are primarily for TerraMind UI, we use deltas for streaming
