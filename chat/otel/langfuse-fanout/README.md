...
TerraMind can send tenant-scoped agent traces to a tenant Langfuse project and
...
- the Go gateway is the only endpoint TerraMind talks to;
...
- TerraMind sends tenant traces to the local fanout gateway when
...
- The collector exports every trace to the central Langfuse project using
  ...
  the TerraMind app derives central score auth from `LANGFUSE_PUBLIC_KEY` and
  `LANGFUSE_SECRET_KEY`.
- The collector also exports tenant-enabled traces to the tenant Langfuse
  ...
  tenant `Authorization` header that TerraMind attaches to the OTLP request.
- For tenant-exportable runs, TerraMind uses a destination-scoped gateway URL
...
- Tenant export is conditional. TerraMind uses a destination-scoped gateway URL
  ...
  auth.
- User feedback scores use Langfuse's direct REST API from the TerraMind API
  process. Central scores use TerraMind's normal central Langfuse env config;
  tenant scores use tenant app configuration when tenant fanout is enabled.

Tenant Langfuse keys are expected to come from TerraMind app configuration.
When available, an authorized administrator can configure and verify the
connection under **Settings > Langfuse**; TerraMind encrypts the secret key at
...
- Langfuse base URLs are startup configuration. `LANGFUSE_FANOUT_CENTRAL_BASE_URL`
  and `LANGFUSE_FANOUT_TENANT_DESTINATIONS` must be known when TerraMind and the
...
- Tenant Langfuse API keys can be added, changed, or disabled in tenant app
  configuration at runtime without restarting TerraMind or the gateway.
...
- `LANGFUSE_FANOUT_TENANT_EXPORT_DISABLED=true` can be set on TerraMind as an
...
- This supports Langfuse Cloud and self-hosted Langfuse as long as each allowed
  tenant base URL is configured at TerraMind/gateway startup. Runtime tenant
...
- Media fanout is not transactional across central and tenant projects. If one
  destination accepts `POST /api/public/media` and another fails, TerraMind sees
...
- The gateway exposes Prometheus metrics at `/metrics` using the same bearer
  token shape as TerraMind. Set `LANGFUSE_FANOUT_METRICS_SECRET`, or provide
...
```dotenv
# Used by TerraMind for central feedback scores. Set this to the same non-EU
...
Then start TerraMind with the fanout override:
...
The override builds the fanout gateway image, sets `LANGFUSE_FANOUT_ENABLED=true`, and points TerraMind at
...
`LANGFUSE_FANOUT_COLLECTOR_URL` into the TerraMind app ConfigMap when they are
...
TerraMind stamps `librechat.tenant.id`, `librechat.langfuse.export_plan`, and
...
TerraMind's accepted tenant-ID grammar.
...
- `LANGFUSE_FANOUT_CENTRAL_BASE_URL` is also consumed by the fanout deployment only.
  For non-EU central feedback scores, set TerraMind's normal `LANGFUSE_BASE_URL`
...
- `LANGFUSE_FANOUT_COLLECTOR_URL` is the local gateway URL used by TerraMind.
