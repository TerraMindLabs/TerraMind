...
import { extractTerraMindParams } from './llm';

describe('extractTerraMindParams', () => {
  it('should return defaults when options is undefined', () => {
    const result = extractTerraMindParams(undefined);
  ...
  it('should return defaults when options is null', () => {
    const result = extractTerraMindParams();
  ...
  it('should extract all TerraMind params and leave model options', () => {
    const options = {
      resendFiles: false,
    ...
    const result = extractTerraMindParams(options);
  ...
  it('should keep imageDetail out of the model options sent to the provider', () => {
    const result = extractTerraMindParams({ model: 'gpt-4o', imageDetail: ImageDetail.low });
  ...
  it('should leave imageDetail undefined when the agent configures none', () => {
    const result = extractTerraMindParams({ model: 'gpt-4o' });

    expect(result.imageDetail).toBeUndefined();
    expect(result.modelOptions).toEqual({ model: 'gpt-4o' });
  });

  it('should handle null values for TerraMind params', () => {
    const options = {
      resendFiles: true,
      promptPrefix: null,
  ...
  it('should only extract known TerraMind params', () => {
    const options = {
      resendFiles: false,
  ...
  it('should not mutate the original options object', () => {
    ...
    extractTerraMindParams(options);

    // Original object should remain unchanged
    expect(options).toEqual(originalOptions);
  });

  it('should handle undefined values for optional TerraMind params', () => {
    const options = {
      resendFiles: false,
