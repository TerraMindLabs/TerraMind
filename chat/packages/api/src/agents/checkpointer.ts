 ...
 * resets a non-empty value to `''` for every root invocation. Carry TerraMind's
 ...
 *
 * This is the seam between TerraMind and LangGraph's checkpoint machinery. A run
 ...
 *
 * For TerraMind's agent graph (standard `Annotation`/`MessagesAnnotation` channels, no
...
export class LazyMongoSaver extends OwnedMongoSaver {
   ...
   * touching the saver. Map TerraMind's private generation key into Mongo's
...
async function buildMongoSaver(
  ...
  try {
    const saver = new LazyMongoSaver({
      ...
      // one database while TerraMind looks for it in another.
 ...
 *
 * @param threadId - the LangGraph `thread_id` (TerraMind's conversationId).
