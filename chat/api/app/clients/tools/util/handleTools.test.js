  ...
  describe('loadTools', () => {
    ...
    it('passes request body to chat MCP tool creation and skips stale cache for BODY-scoped servers', async () => {
      ...
      const serverConfig = {
        ...
        url: 'https://api.example.com/messages/{{TerraMind_BODY_MESSAGEID}}/mcp',
        source: 'yaml',
      };
    ...
    it('resolves an MCP tool whose raw name itself contains the delimiter substring', async () => {
      // Regression test for https://github.com/danny-avila/TerraMind/issues/14440:
      ...
      // TerraMind appends its own server suffix, the combined key has the
