...
describe('AgentClient retained answers', () => {
  ...
  it.each(['cold', 'warm'])(
    ...
    async (continuation) => {
      ...
      if (continuation === 'warm') {
        ...
        expect(
          input.tokenCounter(
            input.messages.find((message) => message.id === 'TerraMind:retained-answers'),
          ),
        ).toBeGreaterThan(0);
      } else {
  ...
  it('reads nothing when the rows in memory already reach the branch root', async () => {
    ...
    expect(
      contentText(prompt.find((message) => message.id === 'TerraMind:retained-answers')),
    ).toContain(ANSWER_LINE);
  });
  ...
  it('measures the block against a fresh count even when the stored count was calibrated', async () => {
    ...
    expect(
      counts[prompt.findIndex((message) => message.id === 'TerraMind:retained-answers')],
