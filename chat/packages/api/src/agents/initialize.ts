  ...
  if (removed > 0) {
    logger.debug(
      webSearchDenied
        ...
        : `[initializeAgent] Removed ${removed} ${provider} native web search tool(s); TerraMind web_search is enabled.`,
    );
  }
...
export type InitializedAgent = Agent & {
  ...
  resendFiles: boolean;
  /** Detail level TerraMind encodes image content blocks with, from the agent's
   ...
   *  registered for this agent. Authoritative TerraMind-only signal of the
...
export interface InitializeAgentParams {
  ...
  /** Explicit deployment allowlist for request types that do not carry TerraMind config on req. */
...
export async function initializeAgent(
  ...
  const { resendFiles, maxContextTokens, imageDetail, modelOptions } = extractTerraMindParams(
   ...
   * against tools TerraMind hasn't shipped yet (Claude Code's `edit_file`,
