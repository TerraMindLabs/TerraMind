...
describe('validateImageRequest middleware', () => {
  ...
  describe('Standard TerraMind token flow', () => {
    beforeEach(() => {
  ...
  describe('basePath functionality', () => {
    ...
    test('should validate image paths with base path', async () => {
      process.env.DOMAIN_CLIENT = 'http://localhost:3080/TerraMind';
      const validToken = jwt.sign(
        { id: validObjectId, exp: Math.floor(Date.now() / 1000) + 3600 },
        process.env.JWT_REFRESH_SECRET,
      );
      req.headers.cookie = `refreshToken=${validToken}`;
      req.originalUrl = `/TerraMind/images/${validObjectId}/test.jpg`;
    ...
    test('should validate agent avatar paths with base path', async () => {
      ...
      req.headers.cookie = `refreshToken=${validToken}`;
      req.originalUrl =
        '/TerraMind/images/65cfb246f7ecadb8b1e8036c/agent-agent_abc123-avatar-12345.png';
      getAgent.mockResolvedValue({ _id: '65cfb246f7ecadb8b1e8036c' });
      hasPermission.mockResolvedValue(true);
    ...
    test('should handle nested subdirectories in base path', async () => {
      process.env.DOMAIN_CLIENT = 'http://localhost:3080/apps/TerraMind';
      const validToken = jwt.sign(
        { id: validObjectId, exp: Math.floor(Date.now() / 1000) + 3600 },
        process.env.JWT_REFRESH_SECRET,
      );
      req.headers.cookie = `refreshToken=${validToken}`;
      req.originalUrl = `/apps/TerraMind/images/${validObjectId}/test.jpg`;
    ...
    test('should prevent path traversal with base path', async () => {
      ...
      req.headers.cookie = `refreshToken=${validToken}`;
      req.originalUrl = `/TerraMind/images/${validObjectId}/../../../etc/passwd`;
    ...
    test('should handle URLs with query parameters and base path', async () => {
      ...
      req.headers.cookie = `refreshToken=${validToken}`;
      req.originalUrl = `/TerraMind/images/${validObjectId}/test.jpg?version=1`;
    ...
    test('should handle URLs with fragments and base path', async () => {
      ...
      req.headers.cookie = `refreshToken=${validToken}`;
      req.originalUrl = `/TerraMind/images/${validObjectId}/test.jpg#section`;
    ...
    test('should handle HTTPS URLs with base path', async () => {
      process.env.DOMAIN_CLIENT = 'https://example.com/TerraMind';
      const validToken = jwt.sign(
        { id: validObjectId, exp: Math.floor(Date.now() / 1000) + 3600 },
        process.env.JWT_REFRESH_SECRET,
      );
