...
/**
 * Helper to write attachment events in Open Responses format (TerraMind:attachment)
 * @param {ServerResponse} res - The server response object
 ...
 * Emits attachments as `TerraMind:attachment` events per the Open Responses extension spec.
 *
 * @param {Object} params
 * @param {ServerRequest} params.req
 * @param {ServerResponse} params.res
  ...
  return async (data, metadata) => {
    ...
    await enqueueCodeOutputBatch({
      ...
      processEntry: async ({ file, sessionId, preparedBuffer, downloadFallback }) => {
         ...
         * and emit a follow-up `TerraMind:attachment` with the same
