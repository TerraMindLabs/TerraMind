...
/**
 * SessionStart lifecycle sources no TerraMind run-construction path emits.
...
const UNAVAILABLE_SESSION_SOURCES: Readonly<Record<string, string>> = Object.freeze({
  compact:
    'SessionStart source "compact" is unavailable because TerraMind PostCompact hook output cannot inject session context',
  clear:
    'SessionStart source "clear" is unavailable because TerraMind has no clear-conversation lifecycle path',
});
...
function getMatcherValidationIssue(
  ...
  if (targetEvent && !QUERY_EVENTS.has(targetEvent) && sourceEvent !== 'SessionStart') {
    return {
      ...
      severity: 'error',
      message: `${targetEvent} does not expose a matcher query in the TerraMind hook runtime`,
    };
  }
  if (matcher.length > MAX_PATTERN_LENGTH || hasNestedQuantifier(matcher)) {
    return {
      ...
      severity: 'error',
      message: 'Matcher exceeds the safe regex limits enforced by the TerraMind hook runtime',
    };
  }
  try {
    ...
    return [
      {
        ...
        severity: 'error',
        message: `${sourceEvent} has no equivalent TerraMind lifecycle event`,
      },
    ];
  }
    ...
    return [
      {
        ...
        severity: 'error',
        message:
          'SubagentStop is unavailable because the TerraMind hook input does not expose stop-hook state',
      },
    ];
  }
    ...
    return [
      {
        ...
        severity: 'error',
        message:
          'PermissionDenied is unavailable because the TerraMind hook output cannot request a retry',
      },
    ];
  }
...
function planMatcher(
        ...
        issues: [
          {
            ...
            severity: 'warning',
            message:
              'Wildcard SessionStart compatibility covers startup and resume; compact and clear never occur in TerraMind',
          },
        ],
      };
    }
      ...
      issues: [
        {
          ...
          severity: 'error',
          message: 'Plugin matcher namespaces require an explicit TerraMind query translation',
        },
      ],
    };
  }
  ...
  if (normalizedTranslation !== matcherSemantics.canonicalMatcher) {
    issues.push({
      ...
      severity: 'warning',
      message: `Plugin matcher "${sourceMatcher}" maps to TerraMind matcher "${matcher}"`,
    });
  }
  if (
