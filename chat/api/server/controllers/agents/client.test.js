    ...
    expect(mockCreateRun.mock.calls[0][0]).toEqual(
      expect.objectContaining({
        ...
        modelCallbacks: expect.arrayContaining([
          expect.objectContaining({
            name: 'TerraMind-model-bound-content-filter',
            raiseError: true,
          }),
          expect.objectContaining({
            name: 'TerraMind-agent-attachment-memory',
          }),
        ]),
      }),
    );
    ...
    expect(processStream.mock.calls[0][1]).toEqual(
      expect.objectContaining({
        configurable: expect.objectContaining({
          ...
          checkpoint_ns: '',
          __TerraMind_checkpoint_ns: '1000',
        }),
      }),
    );
  ...
  it('classifies a terminal chat-model failure without logging provider content', async () => {
    ...
    mockCreateRun.mockImplementation(async (options) => {
      const tracker = options.modelCallbacks.find(
        (callback) => callback.name === 'TerraMind-upstream-model-error-tracker',
      );
      return {
        Graph: null,
        processStream: jest.fn(async () => {
    ...
    expect(processStream).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        configurable: expect.objectContaining({
          ...
          __TerraMind_checkpoint_ns: 'event-actor/fork',
          __TerraMind_event_actor_invocation_id: 'event-2',
          ...
          config: {
            endpoints: {
              [EModelEndpoint.anthropic]: {
                headers: { 'X-Conversation-Id': '{{TerraMind_BODY_CONVERSATIONID}}' },
              },
            },
          },
        };
  ...
  it('allows unmarked model prose and ignores a marked sibling branch on resume', async () => {
    ...
    expect(mockCreateRun.mock.calls[0][0]).toEqual(
      expect.objectContaining({
        tenantId: 'request-tenant',
        modelCallbacks: expect.arrayContaining([
          expect.objectContaining({ name: 'TerraMind-model-bound-content-filter' }),
          expect.objectContaining({ name: 'TerraMind-agent-attachment-memory' }),
        ]),
