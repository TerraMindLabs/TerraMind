...
supplied by TerraMind. Remote callers do not choose `event.source`; provider-specific webhook
...
argument subset. TerraMind reports `applied` only when the exact generation completes with
...
TerraMind resolves the bound agent and child conversation from `(user, tenant, API key, binding)`;
...
TerraMind collects compatible events for up to 750 ms, with a maximum of 8 events and 512 KiB of
