 ...
 */
const INDEX_HTML =
  '<!DOCTYPE html><html lang="en-US"><head><title>TerraMind</title>' +
...
describe('Content Security Policy', () => {
  ...
  it('stamps scripts injected after the shell is read', async () => {
    const response = await request(app).get('/').set('x-TerraMind-enable-query-devtools', '1');
    const nonce = response.headers['content-security-policy']?.match(
      /script-src 'nonce-([^']+)'/,
    )?.[1];

    expect(response.text).toContain('data-TerraMind-query-devtools="true"');
    expect(response.text).toContain(`<script nonce="${nonce}" data-TerraMind-query-devtools`);
  });
