...
function getImporter(jsonData) {
  ...
  if (jsonData.conversationId && (jsonData.messagesTree || jsonData.messages)) {
    logger.info('Importing TerraMind conversation');
    return importTerraMindConvo;
  }
...
/**
 * Imports a TerraMind conversation from JSON.
 *
 ...
 * @returns {Promise<void>} - A promise that resolves when the import is complete.
 */
async function importTerraMindConvo(
  jsonData,
  requestUserId,
  builderFactory = createImportBatchBuilder,
  userRole,
  ...
  try {
    ...
    if (jsonData.recursive) {
      ...
      throw new Error('Invalid TerraMind file format');
    }
    ...
    logger.error(`user: ${requestUserId} | Error creating conversation from TerraMind file`, error);
    throw error;
  }
}
