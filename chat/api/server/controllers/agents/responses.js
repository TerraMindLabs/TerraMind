...
const executeResponse = async (envelope, { req, res }) => {
  ...
  return executeAgentRun({
    ...
    execute: async (execution) => {
       ...
       * Responses API uses its own response-builder shape, so TerraMind-
      ...
      if (actuallyStreaming) {
        ...
        // Use Responses API-specific callback that emits TerraMind:attachment events
...
const listModels = async (req, res) => {
  try {
    ...
    const models = agents.map((agent) => ({
      ...
      owned_by: agent.author ?? 'TerraMind',
      // Additional metadata
      name: agent.name,
 ...
 * The response ID maps to a conversationId in TerraMind's storage.
 *
 * @param {import('express').Request} req
 * @param {import('express').Response} res
 */
